...
![Corra Flowchart](CORRA_FlowChart.png)*Figure 1. Schematic illustration of the proposed research. A radar Cluster-wise Ridge Regression model will be trained using a customized version of CORRA featuring a more accurate snowfall retrieval and used to replace a current component of CORRA. Similarly, a radiometer CRR model will be trained using simulated data to cloud water, and water vapor first-guess estimates conducive to overall improved CORRA products. The proposed ML models can be also used for data assimilation in Deep-Learning based Numerical Weather Prediction (DL NWP).*
...
It should be noted that the CRR approach is applicable to dual-frequency radar observations as well, and can be used to derive the PSD parameters and precipitation rates from the dual-frequency radar observations. The CRR's ability to provide uncertainties estimates enables the sequential inclusion of additional pieces of information (such as path integrated attenuation (PIA) estimates from the surface reference technique (SRT)) through the Ensemble Kalman Smoother module currently in use in CORRA. However, given the relatively short time until the planned release of CORRA V09, and the fact that, for efficiency in navigating the lookup tables, the retrieval of electromagnetic scattering properties at the GMI frequencies is done at the same time with the retrieval of the radar properties from observed Ku-band reflectivity using the Nw-dependent radar profiling algorithm [1,6], the full replacement of the radar-profiling algorithm by a CRR approach would require recoding the GMI scattering property retrievals.  This is a non-trivial task that would require significant testing.  We propose that the radar CRR model be used to derive the $N_w$ ensembles, which will be used to derive the Ka-band radar and GMI observations using the existing physical radar and radiometers. Thus, the computationally intensive first iteration of the radar-profiling algorithm will be replaced by an orders of magnitude faster CRR model in which biases in the ice-phase precipitation are also addressed.  The full replacement of the radar-profiling algorithm by a CRR approach will be considered in the second part of the project and after the finalization of CORRA V09 and is expected to be beneficial to other missions such NASA Atmospheric Observing System (AOS) and  NASA Investigation of Convective Updrafts (INCUS).

A list of proposed tasks for the radar CRR module is provided in Table 1.
